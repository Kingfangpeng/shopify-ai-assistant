...
- **本地 RAG 与安全文件生命周期**：UTF-8/大小/扩展名校验、SHA-256 去重、失败保留旧版本、Milvus 混合检索、7 天回收站和恢复；文档始终作为不可信资料，不进入 system prompt。
- **证据门槛与文件引用**：稠密向量和 BM25 各召回 20 条，经 RRF 融合与 FlashRank 精排后再选择证据；具体型号优先精确匹配，回答保存文件、版本和 chunk 引用。
- **完整的本地安全基线**：Argon2id、HttpOnly/SameSite=Strict Cookie、服务端会话、CSRF、Origin/Host 校验、登录限流、安全响应头和审计事件。
- **可用的产品界面**：模型选择、SSE 流式输出、知识检索与工具活动、文件引用、来源标识、停止/重试、会话持久化，以及桌面和移动端响应式布局。
...
flowchart LR
    ...
    CHAT --> RAG[RAG 服务]
    RAG --> VEC[(Milvus Dense + BM25)]
    RAG --> RANK[RRF + FlashRank]
...
普通问答、知识问答和深度分析共用一次检索结果。界面展示路由、检索、精排、证据选择、模型调用、业务工具和引用校验等可审计活动摘要，不展示模型隐藏思维链。回答完成后，执行过程和引用会随消息保存；点击引用可定位到知识库中的文件和 chunk。

## 会话上下文与自动压缩

SQLite 会保留完整聊天记录，发送给模型的上下文则保持有界：最多注入 12 条、8000 字符的尚未摘要消息，以及最多 2000 字符的版本化会话摘要。普通问答和深度分析共用这一套上下文。

每次回答完成后，若本轮尚未摘要的完整消息达到 12 条或累计达到 8000 字符，系统会用本地模型自动合并已有摘要与新增消息。下一轮只注入该摘要和摘要之后的新消息，避免重复占用上下文；最新消息会优先保留。摘要失败不会删除原始聊天记录，后续回答仍可使用有界的近期消息。

用户在“长期记忆”页面确认的事实会作为独立上下文跨会话生效，最多注入 12 条、3000 字符；候选记忆在确认前不会使用，密码、Token、密钥及支付信息禁止保存。
